...
export class FormulaEngine {
   ...
   * Оцінює формулу
   */
  public evaluateFormula(formula: BonusFormula, data: any, deps: number[]): number {
    switch (formula.type) {
      case 'linear':
        return this.evaluateLinear(formula, data, deps);
      case 'exponential':
        return this.evaluateExponential(formula, data, deps);
      default:
   ...
   */
  private evaluateLinear(formula: BonusFormula, data: any, deps: number[]): number {
   ...
   */
  private evaluateExponential(formula: BonusFormula, data: any, deps: number[]): number {
    // ВАЖЛИВО: deps вже враховані в коефіцієнтах A та B через modifier.formula
    // НЕ потрібно їх ще раз застосовувати!
