...
export class ResourceManager implements SaveLoadManager, IResourceManager {
  ...
  private updateResources(dT: number): void {
    this.resources.forEach((resourceData, _resourceId) => {
      // Додаємо дохід
      if (resourceData.income > 0) {
        resourceData.balance = Math.min(resourceData.max, resourceData.balance + dT*resourceData.income*resourceData.multiplier);
      }
      
      // Віднімаємо витрати
      if (resourceData.consumption > 0) {
        resourceData.balance = Math.max(0, resourceData.balance - dT*resourceData.consumption);
      }
    });
  }
  ...
  addResources(changes: ResourceChange[]): boolean {
    ...
    changes.forEach(change => {
      ...
      if (actualChange !== 0) {
        ...
        this.addToHistory(change.resourceId, actualChange, newAmount, change.reason);
        results.push(true);
      } else {
        results.push(false);
      }
  ...
  spendResources(changes: ResourceChange[]): boolean {
    ...
    changes.forEach(change => {
      ...
      resourceData.balance = newAmount;
              this.addToHistory(change.resourceId, -Math.abs(change.amount), newAmount, change.reason);
    });
  ...
  setResourceAmount(resourceId: ResourceId, amount: number, reason: string = 'manual'): void {
    ...
    resourceData.balance = clampedAmount;
            this.addToHistory(resourceId, clampedAmount - resourceData.balance, clampedAmount, reason);
  }
  ...
  private addToHistory(resourceId: ResourceId, amount: number, balance: number, reason?: string): void {
  ...
  getStats(): {
    ...
    this.resources.forEach((resourceData, _id) => {
